...
The worker supports a controlled CPU-only audio extraction job.

Expected input inside `event["input"]`:
...
- Downloads the original media from S3-compatible RunPod Storage.
- Extracts only the first audio stream with ffmpeg.
- Stores `audio_extract.m4a` at `media/audio/media-<media_file_id>/audio_extract.m4a`.
- Runs ffprobe on the extracted audio for technical metadata.
- Sends a callback with `artifact_type=audio_extract`.

This job does not run Whisper, transcription, OCR, frame extraction, visual analysis, Qwen, MiniCPM, or GPU processing.

## Job whisper_transcribe

Controlled CPU-only Whisper transcription for the already extracted audio artifact.

Decision for the first controlled run:
...
- Device: CPU
- Compute type: int8
- Max audio duration: `WHISPER_MAX_AUDIO_SECONDS`, default `300`
...
{
  "job_id": 22,
  "tipo": "whisper_transcribe",
  "payload_json": {
    "media_file_id": 7,
    "audio_artifact_id": 13,
    "bucket": "zozga7skni",
    "audio_object_key": "media/audio/media-7/audio_extract.m4a",
    "model": "tiny",
    "language": "pt",
    "duration_seconds": 87.541995
  },
  "callback_url": "https://app.meustatus.com/api/runpod/callback"
}
```
...
- `media/transcriptions/media-7/whisper_tiny/transcription.txt`
- `media/transcriptions/media-7/whisper_tiny/transcription.srt`
- `media/transcriptions/media-7/whisper_tiny/transcription_metadata.json`

The callback returns `transcricao_txt` as the main artifact and `transcricao_srt` plus `transcricao_metadata` in `extra_artifacts`.

This job does not use GPU, video input, OCR, frame extraction, Qwen, MiniCPM, or visual analysis.
