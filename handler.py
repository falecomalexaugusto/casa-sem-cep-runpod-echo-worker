...
def _format_srt_timestamp(seconds: float | None) -> str:
...
def _run_whisper_transcribe(input_data: dict[str, Any]) -> dict[str, Any]:
    from faster_whisper import WhisperModel

    payload_json = _get_payload_json(input_data)
    media_file_id = payload_json.get("media_file_id")
    audio_artifact_id = payload_json.get("audio_artifact_id")
    bucket = str(payload_json.get("bucket") or os.environ["RUNPOD_STORAGE_BUCKET"])
    object_key = str(payload_json.get("audio_object_key") or payload_json.get("object_key") or "")
    if not object_key:
        raise ValueError("audio_object_key is required for whisper_transcribe")

    model_name = str(payload_json.get("model") or "tiny")
    language = str(payload_json.get("language") or "pt")
    duration_seconds = payload_json.get("duration_seconds")
    max_audio_seconds = float(os.environ.get("WHISPER_MAX_AUDIO_SECONDS", "300"))
    if duration_seconds is not None and float(duration_seconds) > max_audio_seconds:
        raise ValueError(f"audio duration exceeds WHISPER_MAX_AUDIO_SECONDS={max_audio_seconds}")

    s3 = _build_s3_client()
    base_key = f"media/transcriptions/media-{media_file_id}/whisper_{model_name}"
    txt_key = f"{base_key}/transcription.txt"
    srt_key = f"{base_key}/transcription.srt"
    metadata_key = f"{base_key}/transcription_metadata.json"

    with tempfile.TemporaryDirectory(prefix="whisper-transcribe-") as tmpdir:
        audio_path = str(Path(tmpdir) / "audio_extract.m4a")
        ...
        segments_iter, info = model.transcribe(audio_path, language=language, task="transcribe", vad_filter=False)
        segments = list(segments_iter)

    txt_lines = [segment.text.strip() for segment in segments if segment.text and segment.text.strip()]
    ...
    metadata = {
        ...
        "source_audio_object_key": object_key,
        "model": model_name,
        "language": language,
        ...
        "duration_seconds": getattr(info, "duration", duration_seconds),
        "segment_count": len(segments),
        "max_audio_seconds": max_audio_seconds,
        "segments": [
            {"start": segment.start, "end": segment.end, "text": (segment.text or "").strip()}
    ...
    s3.put_object(Bucket=bucket, Key=txt_key, Body=txt_bytes, ContentType="text/plain; charset=utf-8")
...
def _build_result_json(tipo: str, input_data: dict[str, Any]) -> dict[str, Any]:
    ...
    if tipo == "whisper_transcribe":
        return _run_whisper_transcribe(input_data)
